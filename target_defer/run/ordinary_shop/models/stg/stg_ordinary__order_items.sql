    ...
    create or replace table `dbt-studies-462323`.`ordinary_shop_dbt_prod`.`stg_ordinary__order_items_v1`
  ...
  (
    ...
    select order_item_sk, order_id, product_id, quantity, unit_price, created_at, updated_at, loaded_at
    from (
        with source_data as (
    select 
        order_id
